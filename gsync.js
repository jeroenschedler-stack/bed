/* === gsync.js — BED → Google Sheets (FINAL, prefer Pass B decimals) === */
...
function extractGroupsFromHeading() {
  ...
  if (tryMatch(label.hosp) !== '') out['Hospitality skills'] = tryMatch(label.hosp);
  if (tryMatch(label.bed)  !== '') out['BED competencies']   = tryMatch(label.bed);
  if (tryMatch(label.own)  !== '') out['Taking ownership']   = tryMatch(label.own);
  if (tryMatch(label.coll) !== '') out['Collaboration']      = tryMatch(label.coll);
  return out;
}

/* ---------- PASS B: compute averages from statement scores ---------- */
function extractGroupsFromStatements() {
  ...
  Object.keys(acc).forEach(k => {
    if (acc[k].count > 0)
      out[k] = Number(((acc[k].sum / acc[k].count) / 5 * 100).toFixed(1)); // keep one decimal
  });
  return out;
}
...
function buildPayload() {
  const d = readPDF();
  return { mode: MODE(), timestamp: new Date().toISOString(),
    teamMemberName: d.teamMemberName, teamMemberEmail: d.teamMemberEmail,
    teamMemberLocation: d.teamMemberLocation, peerName: d.peerName,
    peerEmail: d.peerEmail, peerLocation: d.peerLocation,
    overallPct: d.overallPct, groups: d.groups,
    recommendation: d.recommendation, answers: d.answers };
}
